...
def main() -> None:
    ...
    local = getattr(cfg, "DATA_LOCAL_DIR", None)
    if local:
        # Pre-built COCO/roboflow dataset already on the run host (ATR/Gate 3):
        # no download, no conversion — just point rfdetr at it.
        dataset_dir = Path(local).expanduser()
        if not (dataset_dir / "train" / "_annotations.coco.json").is_file():
            raise SystemExit(f"DATA_LOCAL_DIR not a roboflow dataset: {dataset_dir}")
        print(f"[data] using pre-built local dataset {dataset_dir}")
    else:
