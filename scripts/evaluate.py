...
def failure_taxonomy(coco_gt: COCO, results: list[dict], label: str) -> None:
    ...
    by_range = Tally()  # ATR range-study axis (image nominal_range_m field)
    img_range = {im["id"]: im.get("nominal_range_m") for im in coco_gt.dataset["images"]}
    ...
    for img in coco_gt.dataset["images"]:
        ...
        for g, matched in zip(gts, gt_matched):
            ...
            rng = img_range.get(img_id)
            if rng is not None:
                by_range.add((f"range {int(rng):>5}m",), matched)
    ...
    if by_range.total:
        by_range.print_table(f"RECALL BY RANGE ({label})")
